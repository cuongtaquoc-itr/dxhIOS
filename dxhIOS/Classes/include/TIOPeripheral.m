...
NSInteger const MIN_UART_CREDITS_COUNT = 16;
...
- (TIOPeripheral *)initWithCBPeripheral:(CBPeripheral *)cbPeripheral andAdvertisement:(TIOAdvertisement *)advertisement
{
	...
	if (self)
	{
    	...
    	self->_remoteUartMtuSize = [self getRemoteMtuSize];
    	self->_localUartMtuSize  = [self getRemoteMtuSize];
...
- (NSString *)name
{
  if (self.advertisement == nil || [self isEmpty:self.advertisement.localName]) {
...
{
    if(str==nil || str.length==0 || [str isKindOfClass:[NSNull class]] || [str isEqualToString:@""] ||[str isEqualToString:@"(null)"] || [str isEqualToString:@"<null>"]){
        return YES;
    }
    return NO;
}
...
- (void)cancelConnection
{
    ...
    [[STTrace sharedInstance] line:@"cancelConnection %@", [self name]];
...
//    }
	...
	[[TIOManager sharedInstance] cancelPeripheralConnection:self];
//    NSError* error = [NSError errorWithDomain:@"cancelConnection" code:@"cancelConnection" userInfo:nil];
...
- (void)processTIOConnection
{
	...
	else if (self.cbPeripheral.services == nil || self.tioService == nil)
	{
        ...
        [[STTrace sharedInstance] line:@"discoverServices"];
	...
	{
        [[STTrace sharedInstance] line:@"discoverCharacteristics"];
        self.hadToPerformServiceDiscovery = NO;
    ...
    else if ((!self.didSubscribeUARTTxMtu) && (self.uartTxMtuCharacteristic != nil))
    {
        ...
        [[STTrace sharedInstance] line:@"subscribeToCharacteristic uartTxMtuCharacteristic"];
	...
	else if (!self.didSubscribeUARTTxCredits)
	{
        ...
        [[STTrace sharedInstance] line:@"subscribeToCharacteristic uartTxCreditsCharacteristic"];
	...
	else if (!self.didSubscribeUARTTx)
	{
        ...
        [[STTrace sharedInstance] line:@"subscribeToCharacteristic uartTxCharacteristic"];
	...
	else if (!self.didGrantInitialUARTRxCredits)
	{
        ...
        [[STTrace sharedInstance] line:@"grantLocalUARTCredits"];
...
{
    [[STTrace sharedInstance] error:@"didDiscoverServices, error %@", error];
...
{
    [[STTrace sharedInstance] error: @"didDiscoverCharacteristicsForService, error %@", error];
...
- (void)peripheral:(CBPeripheral *)peripheral didWriteValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    {
        NSLog(@"didWriteValueForCharacteristic UART_RX_MTU_UUID");
        if (error == nil)
        {
...
{
    [[STTrace sharedInstance] line:@"didUpdateRSSI %@, error %@", peripheral.RSSI, error];
...
{
    NSObject *obj = [[NSObject alloc] init];
    [[STTrace sharedInstance] line:@"TIOPeripheral-didConnect %p", obj];
...
- (NSInteger)getRemoteMtuSize
{
    //BIOTRES PRO ID STARTS WITH 13
    if ([self.name hasPrefix:@"13"]) {
        NSLog(@"getRemoteMtuSize TIO_MAX_UART_DATA_SIZE_PRO");
        return TIO_MAX_UART_DATA_SIZE_PRO;
    }
    return TIO_MAX_UART_DATA_SIZE;
}
@end
